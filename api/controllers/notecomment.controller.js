...
export const createNoteComment = async (req, res, next) => {
  try {
    ...
    if (userId !== req.user.id) {
      return next(
        errorHandler(403, 'You are not allowed to create this Note comment')
      );
    }
...
export const deleteNoteComment = async (req, res, next) => {
  try {
    ...
    await NoteComment.findByIdAndDelete(req.params.commentId);
...
export const getallnotecomments = async (req, res, next) => {
  if (!req.user.isAdmin)
  ...
  try {
    ...
    const notecomments = await NoteComment.find()
      ...
      .skip(startIndex)
      .limit(limit);
    const totalNoteComments = await NoteComment.countDocuments();
    const now = new Date();
    ...
    const lastMonthRecordComments = await NoteComment.countDocuments({
      createdAt: { $gte: oneMonthAgo },
    });
    res
      .status(200)
      .json({ notecomments, totalNoteComments, lastMonthRecordComments });
  } catch (error) {
    next(error);
  }
};
