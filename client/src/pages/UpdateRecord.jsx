  ...
  useEffect(() => {
    try {
      const fetchRecord = async () => {
        const res = await fetch(`/api/record/getrecords?recordId=${recordId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/record/updaterecord/${formData._id}/${currentUser._id}`,
        {
          method: 'PUT',
          headers: {
  ...
  return (
    <div className='p-3 max-w-3xl mx-auto min-h-screen'>
      <h1 className='text-center text-3xl my-7 font-semibold'>
        Update a Monthly Production Record
      </h1>
